...
// ここを受け取った値で絞り込む
$procards = $db->prepare('SELECT m.name, m.picture, p.* FROM members m, projects p WHERE m.id=p.member_id AND p.place_id = ? ORDER BY p.created DESC LIMIT ?, 6');
$procards->bindParam(1, $_GET['place'], PDO::PARAM_INT);
$procards->bindParam(2, $start, PDO::PARAM_INT);
$procards->execute();
