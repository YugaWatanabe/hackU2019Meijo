...
$search = '%' . $_GET['search'] . '%';
// ここを受け取った値で絞り込む
$procards = $db->prepare('SELECT m.name, m.picture, p.* FROM members m, projects p WHERE m.id=p.member_id AND (p.message LIKE ? OR project_name LIKE ?) ORDER BY p.created DESC LIMIT ?, 6');
$procards->bindParam(1, $search, PDO::PARAM_STR);
$procards->bindParam(2, $search, PDO::PARAM_STR);
...
<body>
    ...
    <div class="container">
        ...
        <ul class="paging">
            <?php if ($page > 1) : ?>
                <li><a href="searchproject.php?search=<?php print(htmlspecialchars($_GET['search'], ENT_QUOTES)); ?>&page=<?php print($page - 1); ?>">前のページへ</a></li>
            <?php else : ?>
                <li>前のページへ</a></li>
            <?php endif; ?>

            <?php if ($page < $maxPage) : ?>
                <li><a href="searchproject.php?search=<?php print(htmlspecialchars($_GET['search'], ENT_QUOTES)); ?>&page=<?php print($page + 1); ?>">次のページへ</a></li>
            <?php else : ?>
                <li>次のページへ</a></li>
            <?php endif; ?>
        </ul>
